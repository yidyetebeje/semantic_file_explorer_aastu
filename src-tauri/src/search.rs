use crate::db::{DbError, TEXT_TABLE_NAME, IMAGE_TABLE_NAME, open_or_create_text_table, open_or_create_image_table};
...
use arrow_array::{Float32Array, StringArray, TimestampSecondArray, Array};
use futures_util::TryStreamExt;
use std::cmp::Ordering;

#[cfg(test)]
use crate::db::{TEXT_EMBEDDING_DIM, IMAGE_EMBEDDING_DIM};
...
pub async fn multimodal_search(
    ...
    #[cfg(test)]
    println!("Search parameters: limit={}, threshold={:?}, filter={:?}", 
    ...
    if search_text {
        ...
        let text_results = search_text_content(&text_table, &query, fetch_limit, score_threshold).await?;
            ...
            Err(e) => {
                ...
                match e {
                    SearchError::ImageEmbeddingError(ref img_err) => {
                        if let Some(file_not_found) = img_err.to_string().strip_prefix("File not found: ") {
    ...
    info!("Multimodal search found {} total results", combined_results.len());
    #[cfg(test)]
    println!("Multimodal search found {} total results", combined_results.len());
...
async fn search_text_content(
    ...
    if embeddings.is_empty() {
        return Err(SearchError::OperationFailed("Failed to generate embedding for query".to_string()));
    }
    ...
    let vector_query = table
        ...
        .select(Select::columns(&["file_path", "content_hash", "chunk_id", "last_modified"]));
    ...
    let record_batches = query_result.try_collect::<Vec<_>>().await
    ...
    for batch in record_batches {
        ...
        let content_hashes = batch.column_by_name("content_hash")
            .and_then(|array| array.as_any().downcast_ref::<StringArray>())
            .ok_or_else(|| SearchError::OperationFailed("Missing content_hash column".to_string()))?;
        
        let last_modified = batch.column_by_name("last_modified")
        ...
        let distances = batch.column_by_name("distance")
...
async fn search_image_content(
    ...
    let embedding = embed_text_for_image_search(query)
        .map_err(|e| {
    ...
    for batch in record_batches {
        ...
        let file_hashes = batch.column_by_name("file_hash")
            .and_then(|array| array.as_any().downcast_ref::<StringArray>())
        ...
        // Optional columns
        let widths = batch.column_by_name("width")
            .and_then(|array| array.as_any().downcast_ref::<arrow_array::Int32Array>());
            
        let heights = batch.column_by_name("height")
            .and_then(|array| array.as_any().downcast_ref::<arrow_array::Int32Array>());
            
        let thumbnail_paths = batch.column_by_name("thumbnail_path")
            .and_then(|array| array.as_any().downcast_ref::<StringArray>());
        ...
        for i in 0..batch.num_rows() {
            ...
            let width = widths.map(|array| if array.is_null(i) { None } else { Some(array.value(i)) }).flatten();
            let height = heights.map(|array| if array.is_null(i) { None } else { Some(array.value(i)) }).flatten();
            let thumbnail_path = thumbnail_paths.map(|array| if array.is_null(i) { None } else { Some(array.value(i).to_string()) }).flatten();
...
mod tests {
    ...
    async fn setup_test_multimodal_db() -> (Connection, TestDb) {
        ...
        let docs = [
            ("test_doc1.txt", "This is a document about artificial intelligence and machine learning", 1.0),
            ("test_doc2.txt", "Database systems and data structures are important in computer science", 2.0),
            ("test_doc3.txt", "Cloud computing offers scalable resources for businesses", 3.0),
        ];
        ...
        for (path, _content, seed) in docs.iter() {
            ...
            upsert_document(&text_table, &file_path, &content_hash, &[embedding]).await.unwrap();
        }
        
        // Add test image documents
        let images = [
            ("photo1.jpg", "A photo of a cat playing with a toy", 1.0),
            ("landscape.jpg", "A beautiful mountain landscape at sunset", 2.0),
            ("chart.png", "A data visualization chart about machine learning", 3.0),
        ];
    ...
    async fn test_multimodal_search_returns_results() {
        ...
        assert!(search_result.is_ok(), "Search function should complete without error");
    ...
    async fn test_search_content_type_filtering() {
        ...
        for result in &text_results {
            assert_eq!(result.content_type, ContentType::Text, "Result should be text type");
        }
